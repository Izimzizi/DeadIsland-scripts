...
uevr.sdk.callbacks.on_pre_engine_tick(function(engine, delta)
   ...
   vr.get_pose(vr.get_right_controller_index(), melee_data.right_hand_pos_raw, melee_data.right_hand_q_raw)
    ...
    -- Check if push gesture is active (set by gesturePushing.lua)
    local isPushActive = _G.GESTURE_PUSH_ACTIVE or false

    -- Skip attack detection if push gesture is active
    if not isPushActive then
    ...
    --print(cur_mon:get_full_name())
        ...
        and not string.find(cur_mon:get_full_name(), "Kick") and not string.find(cur_mon:get_full_name(), "GroundPound") then
            cur_mon.RateScale = 3.0 -- Maximum working value of 5.0. Lower values may work better. 4.0 seems ideal.
        else
